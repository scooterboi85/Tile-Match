...
function onCardFlipped(newlyFlippedCard) {
  ...
  window.setTimeout( removeGlow,350,newlyFlippedCard.element );
  window.setTimeout( removeGlow,350,lastCardFlipped.element );
  if(counters["matches"] === 6)
  {
    winAudio.play();
    let head = document.getElementById("title");
    head.innerHTML = '<h1>You Won!</h1><div id = "buttonDiv" align="center"><button type="button" onclick = "location.reload()">PLAY AGAIN</button></div><br><br>';
  }
  else
  {
    matchAudio.play();
